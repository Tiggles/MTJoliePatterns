...
define trip { global.state = OPEN; getCurrentDateTime@Time( )( currentTime); println@Console( "OPEN " + currentTime )();  resetTO }
    ...
    [ resetTO() ]  {
        synchronized( stateToken ) {
            if ( global.state == OPEN )
            {
                ...
                getCurrentDateTime@Time( )( currentTime); println@Console( "HALF-OPEN " + currentTime )();
                scheduleTimeout@Time( resetTOval { .operation = "closeTO" } )()
            }
        }
    }
    [ closeTO() ] {
        synchronized( stateToken ) {
            if (global.state == HALF_OPEN) {
                ...
                if ( stable ) {
                    getCurrentDateTime@Time( )( currentTime); println@Console( "CLOSED " + currentTime )();
                    global.state = CLOSED
                }
            }
        }
